...
export async function editTaskTitle(id: string, name: string) {
  try {
    const res = await fetch(`${import.meta.env.VITE_DB_URL}/lists/${id}`, {
      method: "PATCH",
      body: JSON.stringify({
        name
      }),
...
export async function changeTodoStatus(id: string, completed: boolean) {
  try {
    const res = await fetch(`${import.meta.env.VITE_DB_URL}/todos/${id}`, {
      method: "PATCH",
      body: JSON.stringify({
